...
router.get("/catalogo", noauth, async (req, res) => {
  ...
  try {
    const productosObtenidos = await productManager.obtenerProductos(
      page,
      limit,
      sort
    );
...
router.get("/admin", noauth, auth, async (req, res) => {
  try {
    const carritosObtenidos = await cartManager.obtenerCarritos();
...
router.get("/home", noauth, auth, async (req, res) => {
  try {
    let productosObtenidos = productManager.obtenerProductos(1, 300, {
      title: 1,
    });
    let categoriasExistentes = categoryManager.obtenerCategorias();
...
router.get("/adminProduct/:pid", noauth, auth, async (req, res) => {
  ...
  try {
    let productosObtenidos = productManager.obtenerProductoPorID(id);
    let categoriasExistentes = categoryManager.obtenerCategorias();
    await Promise.all([productosObtenidos, categoriasExistentes]).then(
      ([productosObtenidos, categoriasExistentes]) => {
